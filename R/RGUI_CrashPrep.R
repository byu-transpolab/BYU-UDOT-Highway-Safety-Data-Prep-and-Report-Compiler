...
location <- read.csv("data/csv/Crash_Data_14-20.csv")
...
rollups <- read.csv("data/csv/Rollups_14-20.csv")
...
filter_CAMS <- function(df){
  ...
  # remove intersection related crashes
  df <- df %>% filter(intersection_related == "N")
}
